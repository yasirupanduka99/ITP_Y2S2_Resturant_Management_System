...
const fs = require("fs");
const PDFDocument = require("../reports_tables/Staff-Report");

router.post("/generatestaffReport", async function (req, res, next) {
    //load cuurent time
    ...
    const staffReportRequest = req.body.staff;
    console.log(staffReportRequest)
    ...
    myDoc
        .fillColor("#444444")
        .fontSize(20)
        .text("Staff Member Report", 110, 57)
        .fontSize(10)
    ...
    const table = {
        headers: ["Staff ID", "Role","Staff Name", "NIC", "Salary"],
        rows: [],
    };

    for (const StaffMember of staffReportRequest) {
        table.rows.push([
            StaffMember.StaffId,
            StaffMember.Role,
            StaffMember.Name,
            StaffMember.NIC,
           "Rs. "+ StaffMember.Salary,
