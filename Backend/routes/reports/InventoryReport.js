...
const fs = require("fs");
const PDFDocument = require("../reports_tables/inventory-Report-Tablet");

router.post("/generateinventoryreport", async function (req, res, next) {
    //load cuurent time
    ...
    const inventoryReportRequest = req.body.inventory;
    console.log(inventoryReportRequest)
    ...
    for (const inventoryItem of inventoryReportRequest) {
        table.rows.push([
            inventoryItem.ProductName,
            "Rs. "+ inventoryItem.UnitPrice,
            inventoryItem.Qty,
            "Rs. "+ inventoryItem.Price
        ]);
    }
    ...
    myDoc.text(`Total Invetory Cost = Rs. ${total.toFixed(2)}`, 45, 120, { align: "left" });
